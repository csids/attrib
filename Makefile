...
build_package:
	...
	docker run --rm --privileged \
		...
		localhost/sc8-csverse:latest /bin/bash -c \
		' \
		cd /; \
		R CMD build /rpkg; \
		cp *.tar.gz /built/; \
...
drat:
	...
	docker run --rm --privileged \
		...
		localhost/sc8-csverse:latest /bin/bash -c 'Rscript -e "drat::insertPackage(fs::dir_ls(\"/built/\", regexp=\".tar.gz\$\"), repodir = \"/drat\", action=\"prune\")"'
...
pkgdown:
	docker run --rm --privileged \
		...
		-v /Volumes/homes/raw996/dev/containers/csids-hub/agent-pipelines/$(PKGNAME)/built:/built \
		-v /Volumes/homes/raw996/dev/git/drat:/drat \
		localhost/sc8-csverse:latest /bin/bash -c 'Rscript -e "devtools::install(\"/rpkg\", dependencies = TRUE, upgrade = FALSE); pkgdown::build_site(\"/rpkg\")"'
